...
d_blst = 500 # m, depth of the ballast
l_slp = 2500 #m, sleeper length
tf = 3000
tw = max([D/3,800])
# ballast bulk densigh 
rhoblst = 19
wblst = rhoblst*tf*d_blst/1e6
# wblst = 1800*g*tf*d_blst/1e9
wslab = 18.6 
weight = 25*tw*D/1e6
...
#horizontal forces
# braking force
hbr = 2+(d_blst+D/2)/1e3  # in mmm
Bkld = Design_Loads.BrakeForce(l, 1,hbr)
Mbk = Bkld.Moment[0]
#traction force
htf = 0.9+(d_blst+D/2)/1e3
Trld = Design_Loads.TractionForce(l,1,htf)
Mtr = Trld.Moment[0]
Mbtp = 5*[Mbk/5]+[-Mtr]
LD300LA = Design_Loads.M300LA(0,x_shift=(l-6.5)/2) 
yMbt = LD300LA['pos']+[LD300LA['pos'][-1]]
Mbt = ActionEffects.MomentLoad(Mbtp,yMbt, l, y)
fig10 = plt.figure(10)
plt.subplot(211)
Design_Loads.pltAxles(LD300LA) # show th axle locations
plt.xlim([0,l])
plt.ylim([0,6])
plt.yticks([])
plt.xlabel(None)
plt.subplot(212)
plt.plot(y,-Mbt.M)
plt.yticks([-500,0,500,1000,1500],[500,0,-500,-1000,-1500])
plt.xlim([0,l])
...
while x_shift<=l-6.5:
    ...
    LADist = Design_Loads.LoadDist(l, 1, 1, hf/1000, d_blst/1000, l_slp/1000).M300LADist(LD300LA)
...
print('Maximum moment occurs when the shift of the axle group is {:.2f}m'.format(x_shift0))
print('Maximum shear force is {:.2f} kN'.format(max(Vdmax)))
#longitudinal location where the maximum is reached
xmax = locmax[ind]
print('The location along the beam where the maximum moment is reached is {:.2f}m'.format(xmax))
...
# ld300la = Design_Loads.M300LA(0,x_shift0)
# ld300dst = Design_Loads.LoadDist(l, 1, 1, hf/1e3, d_blst/1e3, l_slp/1e3).M300LADist(ld300la)
# Md = np.zeros(len(y))
# for i in range((len(ld300la['pos']))):
